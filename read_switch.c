/**
 * @file [read_switch.c]
 * @brief [This file reads from the gpio17 value file and prints the state of the switch to the console, using software debouncing to ensure the determined state of the switch is correct]
 * @author [Jacob Allenwood]
 * @date [September 29, 2015]
 * @setup [Raspberry Pi 2 - gpio17 is connected between the switch and the pulldown resistor to ground]
 */
...
void print_switch_state(SWITCH_T * S);	// print the state of the switch to the console
// ---------------------------------------------------------
...
/**
 * @brief [initalize switch state structure]
...
/**
 * @brief [initializes gpio17 for reading its value file]
 * @details [open write and close the enable and direction files for gpio17]
...
int get_debounce_vals(SWITCH_T * S) {
		...
		for(i = 0; i < 10; i++) {
			...
			S->values[i] = (read_buffer[0] - 48); // by default the value for high is 49 and value for low is 48
...
/**
 * @brief [print the state of the switch (pressed or released)]
 * @details [the switch state is determined by the condition that the most 10 recent values read from the values file (found in S->values array) are either all high or all low]
 * @return [void]
 */
void print_switch_state(SWITCH_T * S) {
